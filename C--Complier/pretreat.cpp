...
//状态转换表见pretreat.h
int delzs(fstream &in, fstream &out)
{
	char temp ,pre;
	...
	while (!in.eof())
	{
		...
		switch (status)
		{
			...
			case 3:
				...
				{
					fout << "出现注释嵌套！" << endl;
					fout << "Probable Reason: 不支持注释嵌套。" << endl;
					out << "[Error Occured!]";
	...
	{
		out << "/*" << temp_s;	//fout << "/*" << temp_s;
	}
	return 0;
}

//状态转换表见pretreat.h
int delblank(fstream &in, fstream &out)
{
	char temp, pre;
	int status=0;
	in >> noskipws;
	...
	while (!in.eof())
	{
		...
		switch (status)
		{
			...
			case 2:
				...
				{
					status = 1;
					out << pre << temp;		//fout << pre << temp;			//2状态接收到一个非空白字符 则输出最近的一个空白字符
				}
				if (pre!='\n')
...
//消除左递归以及公共左因子
int delleftrecursion(fstream &in, fstream &out)
{
...
//将语法根据|拆分
int splitgrammar(fstream &in, fstream &out)
{
