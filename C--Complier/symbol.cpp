...
//生成符号表
int calsymboltable(fstream &in, fstream &out)
{
	vector<Token> tokens;
	Token token;
	...
	for (titer = tokens.begin();titer != tokens.end();titer++)
	{
		if (titer->type == "IDF")	//当前token为函数名,分为函数的定义和调用情况 
		{
			...
			else if (flag1>0)   //说明是函数内部对函数的调用
			{
				...
				{
					fout << "Undefined Function [" << titer->re << "] at Line " << titer->line << " Colume " << titer->colume << endl;
					return 1;
				}
		...
		else if (titer->type == "ID")											//当前token为变量名，分为变量的声明和调用情况 
		{
			if ((titer - 1)->re == "void" || (titer - 1)->re == "int")		//前一个token为int或void，说明当前为变量的声明 
			{
				if (flag1 == 0 || (flag1 > 0 && flag2 == 0)) 				//变量声明只能在函数获取形参中或不在其他位于函数内部的小括号内 
				{
					...
					{
						fout << "Variable [" << titer->re << "] has been defined!" << endl;
						return 1;
						exit(0);
					}
				}
			...
			else															//前一个token不是int或void，说明为函数的调用 
			{
				...
				{
					fout << "Undefined Variable [" << titer->re << "] at Line " << titer->line << " Colume " << titer->colume << endl;
					return 1;
					//exit(0);
				}
				/* else 														//符号表中存在
