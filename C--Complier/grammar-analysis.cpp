...
//计算终结符集合以及非终结符集合
int cal_vn_vt(fstream &in,fstream &out)
{
			...
			else
			{
				...
				while (1)
				{
					if (k >= gmaper->second.size())
						break;
					//fout << k << " " << gmaper->second.size() << endl;
...
//计算first follow集
int cal_first_follow(fstream &in, fstream &out)
{
	cal_first();
	cal_follow();
	output_ff(out);
...
//计算预测分析表
int cal_mtable(fstream &in, fstream &out)
{
	string name, vnname;
	int emptyflag, i, j;
	init_mtable(mtable);
...
{
	return stack.st[stack.st.size() - 1];
}

//栈顶元素出栈
int stack_popout(struct Stack &stack)
{
	string str;
	if (stack.st.size() == 0)
	{
		fout << "ERROR:栈已为空,无法出栈!" << endl;
		return 1;
	}
	else
	{
...
//生成语法分析树
int make_tree(fstream &in, fstream &out)
{
	//nodenum = 0;
	nodenum = 0;
	string str;
	...
	while (!in.eof())
	{
		...
		{	//直到栈顶元素为终结符 跳出循环 准备比较\出栈操作
			...
			if (mtable.mtable[mtable.vnname[str]][num] == 0)
			{
				...
				fout << "Grammar Error at Line " << token.line << " Colume " << token.colume << endl;
				fout << "Probable Reason: No match for token " << token.re << endl;
				//fout << str << endl;
				//fout << token.re << endl;
			...
			else
			{
				...
				if (stack_popout(stack)) {
					fout << "Probable Reason: Extra characters which can't be recognized." << endl;
					return 1;
				};
				cur->bitnum = 0;
				cur->mtnum = line;
		...
		if (token.re != stack_gettop(stack) && token.type != stack_gettop(stack))
		{
			out << "Grammar Error at Line " << token.line << " Colume " << token.colume << endl;
			fout << "Grammar Error at Line " << token.line << " Colume " << token.colume << endl;
			fout << "Probable Reason: Still Waiting for " << stack_gettop(stack) << endl;
			return 1;
			exit(0);
...
// formula_1 to forula_12 all return true.
...
//formula_14 to formula38 all return true.
...
//生成中间代码
int midcode(fstream &in, fstream &out)
{
	treebit *temp;
	init_funcmap();
	temp = treenode;
